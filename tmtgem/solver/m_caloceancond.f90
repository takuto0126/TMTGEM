...
!################################################################### setoceancond
!# coded on 2018.11.13
subroutine setoceancond(g_mesh,g_param,h_ocean,g_cond) ! 2024.02.01
implicit none
type(mesh),            intent(inout)  :: g_mesh  ! m_mesh_type.f90
type(param_forward),   intent(in)     :: g_param ! m_param.f90
type(param_cond),      intent(in)     :: g_cond  ! m_param.f90 2024.02.01
...
!#[0]## gen default cmodel
 ...
 if ( g_cond%condflag == 1 ) then  ! 2024.02.01
   do i= 1, g_cond%nphys2          ! 2024.02.01
    cmodel(ntet - g_cond%nphys2 + i ) = g_cond%sigma(i) ! 2024.02.01
   end do                          ! 2024.02.01
 end if                            ! 2024.02.01
