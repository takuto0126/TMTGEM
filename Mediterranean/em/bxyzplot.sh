...
dt=`head -8 ./mediterranean.ctl | tail -1 | awk -F"|" '{print($2)}' `
echo "dt=" $dt "[sec]"
...
gmt psxy "$polygongmt" -JX$scl -R$WESN -m -K -L -O -W1,black >> $out21

gmt psxy "$pos5file" -JX$scl -R$WESN -K -L -O -W0.5,black >> $out21

gmt grdcontour $grdtopo -A1000 -C1000 -L-8000/-1000 -W0.1,green -JX -K -O >> $out21
