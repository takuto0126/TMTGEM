...
!----------------------------------------------------------------------
      ...
      REAL TIME !, TEMP(LO%NX) commented out 2019.03.06 Takuto Minami
      REAL, ALLOCATABLE :: TEMP(:) ! 2019.03.06 Takuto MINAMI
	  COMMON /CONS/ ELMAX,GRAV,PI,R_EARTH,GX,EPS,ZERO,ONE,NUM_GRID,	&
					NUM_FLT,V_LIMIT,RAD_DEG,RAD_MIN

	  ALLOCATE(TEMP(LO%NX)) ! 2019.03.06 Takuto MINAMI
	  TEMP = 0.0
	  ...
	  DO K = 1, NUM_FAULT
	     IF (FLT(K)%T0.GE.TIME .AND. FLT(K)%T0.LT.TIME+LO%DT) THEN
			...
			IF (LO%LAYSWITCH .EQ. 0) THEN
			   DO I=1,LO%NX; DO J=1,LO%NY ! 2019.03.06 Takuto Minami
!			   LO%Z(:,:,1) = LO%Z(:,:,1) + LO%DEFORM(:,:)
			   LO%Z(I,J,1) = LO%Z(I,J,1) + LO%DEFORM(I,J) ! 2019.03.06 Takuto Minami
                     END DO ; END DO ! 2019.03.06 Takuto Minami
			ENDIF
			DO I = 1,NUM_GRID
