...
#------------------------------------------------------####
WESN=138.5/153/29/46.5
grdf="eta.grd"
scl=0.6
#topogrd="topo.grd"
CPT="eta.cpt"
#makecpt -Crainbow -T-30/30/0.1 -V > ${CPT}
gmt makecpt -Cpolar -T-1/1/0.05 -V > ${CPT}
...
gmt grdimage $grdf -Ba5f1WeSn -C${CPT} -Jm${scl} -K -R${WESN} -V  -X3 -Y1 > $out21
...
gmt psscale -D9.5/5/8/0.3 -B0.5  -C$CPT -K -O -V >> $out21
gmt psxy -R -Jm -Sc0.1 -G255/0/255 -K -O << EOF >> $out21
159.9518        41.1026
144.807595      39.0582
EOF
gmt pstext -JX17/19 -R0/17/0/19  -G255 -O -V <<EOF >> $out21
